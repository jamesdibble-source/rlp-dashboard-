...
| `engine/daily-scrape.sh` | Daily cron: Domain buy+sold → REA buy+sold → clean → build → deploy. All timeouts + error handling. Always exits 0. |
...
- Runs `daily-scrape.sh` — scrapes all sources, cleans, rebuilds dashboard, deploys
