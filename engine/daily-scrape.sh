#!/bin/bash
# Production daily scrape pipeline — bulletproof, never crashes
# Runs: Domain buy + sold, REA buy + sold, clean, build, deploy
# Cron: 0 16 * * * bash /root/.openclaw/workspace/rlp-project/engine/daily-scrape.sh >> /tmp/daily-scrape.log 2>&1
# (16:00 UTC = 3am AEST / 2am AEDT)
...
if [ -f "$APIFY_TOKEN_FILE" ]; then
  ...
  # REA BUY
  for STATE in VIC NSW QLD WA SA; do
    echo "[$(date '+%H:%M')] REA buy $STATE..."
    timeout 3600 node engine/scrape-rea.js $STATE buy 25 2>&1 | tail -3 || echo "  REA buy $STATE failed (non-fatal)"
    rm -f "engine/data/rea-progress-${STATE,,}-buy.json"
  done
  
  # REA SOLD
  for STATE in VIC NSW QLD WA SA; do
    echo "[$(date '+%H:%M')] REA sold $STATE..."
    timeout 3600 node engine/scrape-rea.js $STATE sold 25 2>&1 | tail -3 || echo "  REA sold $STATE failed (non-fatal)"
