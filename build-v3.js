...
function getData(stateFilter) {
  ...
  const totals = q1(`SELECT COUNT(*) as total, COUNT(CASE WHEN status='sold' THEN 1 END) as sold, COUNT(CASE WHEN status='listing' OR status='likely_sold' THEN 1 END) as listings FROM lots WHERE price>0 AND price<5000000 AND is_outlier=0 ${w}`);
  
  // Averages from last 12 months only
  const avgs = q1(`SELECT ROUND(AVG(price)) as avg_price, ROUND(AVG(lot_size)) as avg_size, ROUND(AVG(price_per_sqm),2) as avg_rate FROM lots WHERE price>0 AND price<5000000 AND is_outlier=0 AND (sold_date >= date('now','-12 months') OR (status != 'sold' AND (list_date >= date('now','-12 months') OR first_seen >= date('now','-12 months')))) ${w}`);
  totals.avg_price = avgs.avg_price || 0;
  totals.avg_size = avgs.avg_size || 0;
  totals.avg_rate = avgs.avg_rate || 0;
  
  // Medians: current listings first, fall back to sold in last 6 months if <10 listings
  function medianOf(arr) { return arr.length ? arr.sort((a,b)=>a-b)[Math.floor(arr.length/2)] : 0; }
  
  let curPrices = q(`SELECT price FROM lots WHERE price>0 AND price<5000000 AND is_outlier=0 AND status!='sold' ${w} ORDER BY price`).map(r=>r.price);
  let curSizes = q(`SELECT lot_size FROM lots WHERE lot_size>0 AND lot_size<5000 AND is_outlier=0 AND status!='sold' ${w} ORDER BY lot_size`).map(r=>r.lot_size);
  let curRates = q(`SELECT price_per_sqm FROM lots WHERE price_per_sqm>0 AND price_per_sqm<5000 AND is_outlier=0 AND status!='sold' ${w} ORDER BY price_per_sqm`).map(r=>r.price_per_sqm);
  
  if (curPrices.length < 10) curPrices = q(`SELECT price FROM lots WHERE price>0 AND price<5000000 AND is_outlier=0 AND (status!='sold' OR sold_date >= date('now','-6 months')) ${w} ORDER BY price`).map(r=>r.price);
  if (curSizes.length < 10) curSizes = q(`SELECT lot_size FROM lots WHERE lot_size>0 AND lot_size<5000 AND is_outlier=0 AND (status!='sold' OR sold_date >= date('now','-6 months')) ${w} ORDER BY lot_size`).map(r=>r.lot_size);
  if (curRates.length < 10) curRates = q(`SELECT price_per_sqm FROM lots WHERE price_per_sqm>0 AND price_per_sqm<5000 AND is_outlier=0 AND (status!='sold' OR sold_date >= date('now','-6 months')) ${w} ORDER BY price_per_sqm`).map(r=>r.price_per_sqm);
  
  totals.med_price = medianOf(curPrices);
  totals.med_size = medianOf(curSizes);
  totals.med_rate = Math.round(medianOf(curRates));
